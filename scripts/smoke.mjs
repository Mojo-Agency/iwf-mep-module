...
check(`all ${total} cards`, $$('#mep-module .mep-card').length === total, String($$('#mep-module .mep-card').length));
...
check(`FI filter -> ${fiCount}`, $$('#mep-module .mep-card').length === fiCount);
...
await sleep(250);
check('search 1 result', $$('#mep-module .mep-card').length === 1);
input.value = 'ääl';
input.dispatchEvent(new window.Event('input', { bubbles: true }));
await sleep(250);
...
check('writing to name', $('.mep-writing-to__who').textContent === 'Mika Aaltola, Finland', $('.mep-writing-to__who').textContent);
...
check('bulk writing to', $('.mep-writing-to__who').textContent === `All ${fiCount} MEPs in Finland`, $('.mep-writing-to__who').textContent);
...
check('clear filters', $$('#mep-module .mep-card').length === total && $('#mep-country').value === '');
