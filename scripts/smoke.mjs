...
check('subject prefilled from template', $('#mep-subject').value === 'A request from someone you represent on the Child Sexual Abuse Regulation');
...
click(templates[1]);
check('template switch (body + subject)', templates[1].getAttribute('aria-pressed') === 'true' && $('#mep-body').value.includes('63%') && $('#mep-body').value.length < 1500 && $('#mep-subject').value === 'Please support a strong Child Sexual Abuse Regulation');
...
check('message label above body', $('.mep-preview__message .mep-preview__key').textContent === 'Message' && $('.mep-preview__body--message').textContent === 'Hello,\nline two');
check('share links present without personal data', $$('.mep-share__link').length === 3 && $$('.mep-share__link').every((a) => !/aaltola|europarl/i.test(a.href)));
check('copy buttons carry an icon', $$('#mep-module .mep-send__row .mep-btn--outline .mep-icon').length === 2);
...
await sleep(2100);
check('label restored with icon', byText('#mep-module button.mep-btn', 'Copy email address') && byText('#mep-module button.mep-btn', 'Copy email address').querySelector('.mep-icon'));
...
check('bulk hint replaces mailto', byText('#mep-module .mep-note', 'Too many addresses'));
